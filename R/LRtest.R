LRtest <-
function(d,z,use.commonSigma=FALSE,id=NULL){
    ...
    #############################################################
    # likelihood ratio test statistic with estimated rho or rho=0
    #############################################################
    ...
    LR0[,2+R] = ifelse(lrt0 < 20, 1-pchisq(lrt0,R-1), -pchisq(lrt0,1,log.p=TRUE));
